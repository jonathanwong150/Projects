...
def get_mine_count(i,k):
    ...
    for a in range(i - 1, i + 2):
        for b in range(k - 1, k + 2):
            if not (a is i and b is k) and 0 <= a < len(board) and 0 <= b < len(board[0]) and board[a][b] is '*':
                mCount += 1
    return mCount
...
def show_space(x, y):
    if dispBoard[x][y] is not None:
        return
    dispBoard[x][y] = board[x][y]
    if board[x][y] is 0:
        for a in range(x - 1, x + 2):
            for b in range(y - 1, y + 2):
                if not (a is x and b is y) and 0 <= a < len(board) and 0 <= b < len(board[0]):
                    show_space(a, b)
    global gameOver
    global clicked
    if(board[x][y] is '*'):
...
def click_space(event):
    ...
    if(len(board)*len(board[0]) - n is clicked): print("You Win")
...
def game():
    ...
    root.mainloop()
    if(gameOver or (width*height - n is clicked)): return
